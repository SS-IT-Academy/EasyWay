module Capybara
  module BootstrapDatepicker
    def select_date(value, datepicker: :bootstrap, format: nil, from: nil, xpath: nil, **args)
      ...
      value = format.present? ? Date.strptime(value, format) : Date.parse(value) unless value.respond_to? :to_date
    ...
    def select_bootstrap_date(date_input, value, format: nil)
      ...
      value = format.present? ? Date.strptime(value, format) : Date.parse(value) unless value.is_a? Date
      ...
      new_value = format.present? ? Date.strptime(date_input.value, format) : Date.parse(date_input.value)

      fail if new_value != value
    end
