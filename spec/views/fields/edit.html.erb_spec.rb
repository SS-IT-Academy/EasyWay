...
describe "fields/edit", type: :view do
  before(:each) do
  ...
  it "renders the edit field form" do
    ...
    assert_select "form[action=?][method=?]", field_path(@field), "post" do
      ...
      assert_select "select#field_field_type_id[name=?]", "field[field_type_id]"
      assert_select "select#field_resource_type_id[name=?]", "field[resource_type_id]"
    end
  end
end
